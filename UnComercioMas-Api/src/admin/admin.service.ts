...
export class AdminService { // <--- FALTABA LA PALABRA 'EXPORT'
  ...
  async assignBranch(dto: AssignBranchDto) {
    ...
    const detallesActuales = vendedor.detalles_vendedor || {};

    const nuevosDetalles = {
      ...detallesActuales,
      sucursalId: sucursal.id,
      sucursalNombre: sucursal.nombre, // Almacenamos el nombre para reportes rápidos
      fechaAsignacion: new Date().toISOString()
    };

    // 5. Actualizar el registro del vendedor con los nuevos detalles JSONB
    return this.usersService.updateUser(vendedor.id, {
      detalles_vendedor: nuevosDetalles
    });
  }
