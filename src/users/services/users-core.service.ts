...
export class UsersCoreService {
  ...
  async findUsersByRole(role: UserRole): Promise<User[]> {
    return this.userRepository.find({
      ...
      // 1. OBLIGATORIO: Cargar la relación 'sucursal'
      // Esto asegura que el objeto 'sucursal' (Branch) se adjunte al usuario.
      relations: ['sucursal'],
      select: {
        id: true,
        email: true,
        ...
        // AÑADIR LA NUEVA PROPIEDAD ESCALAR
        fecha_asignacion: true, // <-- NUEVO CAMPO
      }
      // NOTA: No necesitamos incluir 'sucursal: true' aquí.
      // TypeORM lo incluye automáticamente si está en 'relations'.
    });
  }
